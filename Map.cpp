...
Entity* Map::createEntityFromMap(int z, int x, int y)
{
	...
	if (z == GridColor::spikyTurtle)
		entity = new DeathBlock("spikyTurtle", 0.0, x_pos, y_pos, Vector2f(cellSize, cellSize), Vector2f(x, y));
	if (z == GridColor::lava1)
		entity = new DeathBlock("lava", 0.0, x_pos, y_pos, Vector2f(cellSize, 3*cellSize), Vector2f(x, y));
	if (z == GridColor::spike)
		entity = new DeathBlock("spike", 0.0, x_pos, y_pos, Vector2f(cellSize, 3 * cellSize), Vector2f(x, y));
	return entity;
}
