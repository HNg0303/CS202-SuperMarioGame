...
Entity* Map::createEntityFromMap(int z, int x, int y)
{
	...
	if (z == GridColor::fireBar)
		entity = new Elevator("fireBar", 0.0, 1.0f, x_pos, x_pos, y_pos, (cellSize * y - 4 * cellSize / 2.0f), Vector2f(cellSize, cellSize), Vector2f(x, y));
	...
	if (z == GridColor::spikyTurtle)
		entity = new Block("spikyTurtle", 0.0, x_pos, y_pos, Vector2f(cellSize, cellSize), Vector2f(x, y));
	...
	if (z == GridColor::spike)
		entity = new Block("spike", 0.3, x_pos, y_pos, Vector2f(cellSize, cellSize), Vector2f(x, y));
	if (z == GridColor::pipe)
