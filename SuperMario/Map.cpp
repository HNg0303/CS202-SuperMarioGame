...
Entity* Map::createEntityFromMap(int z, int x, int y)
{
	...
	if (z == GridColor::fireBar)
		entity = new Elevator("fireBar", 0.0, 0.3f, x_pos, x_pos, y_pos, (cellSize * y - (4.0f * cellSize) / 2.0f), Vector2f(cellSize, cellSize), Vector2f(x, y));
	...
	if (z == GridColor::spikyTurtle)
		entity = new Block("spikeyTurtle", 0.0, x_pos, y_pos, Vector2f(cellSize - 0.05f, cellSize - 0.05f), Vector2f(x, y));
