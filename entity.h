...
class DeathBlock : public Unmoveable, public ContactListener { //die if touch
public:
	b2Fixture* fixture = nullptr;
	DeathBlock(string name_i, double frameDuration_i, float x, float y, Vector2f size, Vector2f coords) :
		Unmoveable(name_i, frameDuration_i, x, y, coords) {
		this->size = size;
	}
	void Begin() override;
